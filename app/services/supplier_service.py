...
dc.country  as Country_Region,da.address as Address,dct.email as Email,dct.website as Website,dct.phone as Phone
from 
(
select id,name,ap_supplier_id from {schema_name}.dim_supplier
where id = {supplier_id} and name = '{supplier_name}'
)ds
left join {schema_name}.address_supplier_mapping asm
...
def get_reason_2(supplier_name,Country):
    try:
        text='Please enter'
        if not supplier_name:
            text+=' Supplier Name'
...
def insert_suppliers_data_fun_2(new_dbobj,input_payload):
    try:
        set_env_var()
        input_payload_list=input_payload
        ...
        # contact_input_df_invalid_rows['Reason']= contact_input_df_invalid_rows.apply(lambda x: get_reason(x.supplier_name, x.Country, x.level_1, x.Email), axis=1)
        contact_input_df_invalid_rows['Reason']= contact_input_df_invalid_rows.apply(lambda x: get_reason_2(x.supplier_name, x.Country), axis=1)
...
def insert_suppliers_data_fun_background_task_2(new_dbobj,input_payload):
    try:
        print('inside background task')
        set_env_var()
        input_payload_list=input_payload
